# Pulse — социална мрежа

Социална мрежа, в която потребителите публикуват снимки и мисли, следват се,
коментират и харесват. Коментарите минават през AI модерация, преди да бъдат
публикувани, а AI може да предложи описание по качена снимка.
...
API тестове с Vitest и supertest — 15 теста върху автентикацията, правата
(кой какво може да трие и редактира), харесванията и пагинацията.
...
Тестовете работят срещу **отделна база** (`social_test`), а AI извикванията са
изключени — външна услуга прави тестовете бавни и недетерминирани.

Преди първо пускане схемата трябва да е синхронизирана и с тестовата база:

```
DATABASE_URL="$(grep '^DATABASE_URL' .env | cut -d '"' -f2 | sed 's|/social?|/social_test?|')" npx prisma db push
```

## Предстои

- Профилни снимки, стена, редакция на пост, видимост
